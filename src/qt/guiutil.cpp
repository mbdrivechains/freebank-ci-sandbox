...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a FreeBank address (e.g. %1)").arg(
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FreeBank.lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("FreeBank (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitassets.desktop";
    return GetAutostartDir() / strprintf("bitassets-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitAssets\n";
        else
            optionFile << strprintf("Name=BitAssets (%s)\n", chain);

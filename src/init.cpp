 ...
 * between freebankd, and bitcoin-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (showDebug) {
        strUsage += HelpMessageOpt("-acceptnonstdtxn", strprintf("Relay and mine \"non-standard\" transactions (regtest only)"));
...
bool AppInitMain()
{
    ...
    int64_t nBillDBCache = nTotalCache / 8;
    if (nBillDBCache > (1 << 21))
        nBillDBCache = (1 << 21);
    nTotalCache -= nBillDBCache;

...
struct MainSignalsInstance {
    ...
    boost::signals2::signal<void (const std::map<uint256, BitAssetTransactionData>&, const std::shared_ptr<const CBlock> &, const CBlockIndex *pindex, const std::vector<CTransactionRef>&)> BlockConnected;
...
void CMainSignals::BlockConnected(const std::map<uint256, BitAssetTransactionData>& mapAssetData, const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindex, const std::shared_ptr<const std::vector<CTransactionRef>>& pvtxConflicted) {
    m_internals->m_schedulerClient.AddToProcessQueue([mapAssetData, pblock, pindex, pvtxConflicted, this] {
        m_internals->BlockConnected(mapAssetData, pblock, pindex, *pvtxConflicted);
    });
}

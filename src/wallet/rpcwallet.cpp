...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart freebankd with -deprecatedrpc=addwitnessaddress.\n"
...
UniValue issuebill(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 6)
        throw std::runtime_error(
            "issuebill\n"
            "\nArguments:\n"
            "1. \"body\"            (string, required) hex of the encrypted bill body\n"
            "2. \"amount\"          (numeric or string, required) face amount\n"
            "3. \"escrow\"          (numeric or string, required) escrow bond amount\n"
            "4. \"maturityheight\"  (numeric, required) sidechain height the bill matures at\n"
            "5. \"graceblocks\"     (numeric, optional) grace period, default 1008\n"
            "6. \"fee\"             (numeric or string, optional) default 0.001\n"
            "\nIssue (draw + accept) a bill of exchange. v1 single-wallet handshake:\n"
            "this wallet plays drawer, acceptor and initial holder with fresh keys.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nExamples:\n"
            + HelpExampleCli("issuebill", "\"deadbeef\" 10 7.5 500")
            + HelpExampleRpc("issuebill", "\"deadbeef\" 10 7.5 500")
        );

    ObserveSafeMode();

    std::string strBody = request.params[0].get_str();
    if (!IsHex(strBody))
        throw JSONRPCError(RPC_MISC_ERROR, "Invalid body (need hex)");
    std::vector<unsigned char> vchBody = ParseHex(strBody);

    CAmount nAmount = AmountFromValue(request.params[1]);
    CAmount nEscrow = AmountFromValue(request.params[2]);
    int nMaturityHeight = request.params[3].get_int();
    if (nMaturityHeight <= 0)
        throw JSONRPCError(RPC_MISC_ERROR, "Invalid maturity height");

    uint32_t nGraceBlocks = DEFAULT_BILL_GRACE_BLOCKS;
    if (request.params.size() >= 5)
        nGraceBlocks = request.params[4].get_int();
    ...
    CTransactionRef tx;
    std::string strFail = "";
    if (!pwallet->IssueBill(tx, strFail, vchBody, nAmount, nEscrow, nMaturityHeight, nGraceBlocks, nFee)) {
        LogPrintf("%s: %s\n", __func__, strFail);
        throw JSONRPCError(RPC_MISC_ERROR, strFail);
    }
    ...
    response.pushKV("bill_id", BillIDFromBody(vchBody).ToString());
    return response;
}

UniValue endorsebill(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. \"topubkey\"  (string, required) endorsee pubkey (33-byte compressed hex)\n"
            "3. \"fee\"       (numeric or string, optional) default 0.001\n"
            "\nEndorse (transfer) a bill of exchange to a new holder.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nExamples:\n"
            + HelpExampleCli("endorsebill", "1 \"02...\"")
            + HelpExampleRpc("endorsebill", "1 \"02...\"")
        );
    ...
    if (!IsHex(strPubKey))
        throw JSONRPCError(RPC_MISC_ERROR, "Invalid endorsee pubkey (need hex)");
    std::vector<unsigned char> vchToPubKey = ParseHex(strPubKey);
    ...
    uint256 txid;
    std::string strFail = "";
    if (!pwallet->EndorseBill(strFail, txid, nBillID, vchToPubKey, nFee)) {
        LogPrintf("%s: %s\n", __func__, strFail);
        throw JSONRPCError(RPC_MISC_ERROR, strFail);
    }
...
UniValue retirebill(const JSONRPCRequest& request)
{
    ...
    uint256 txid;
    std::string strFail = "";
    if (!pwallet->RetireBill(strFail, txid, nBillID, nFee)) {
        LogPrintf("%s: %s\n", __func__, strFail);
        throw JSONRPCError(RPC_MISC_ERROR, strFail);
    }
...
UniValue claimbillescrow(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            + HelpExampleRpc("claimbillescrow", "1")
        );
    ...
    uint256 txid;
    std::string strFail = "";
    if (!pwallet->ClaimBillEscrow(strFail, txid, nBillID, nFee)) {
        LogPrintf("%s: %s\n", __func__, strFail);
        throw JSONRPCError(RPC_MISC_ERROR, strFail);
    }
...
UniValue getnewbillpubkey(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size())
        throw std::runtime_error(
            "getnewbillpubkey\n"
            "\nGet a fresh compressed pubkey for receiving a bill (as drawer,\n"
            "endorsee or holder). Fresh key per bill is the privacy default.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nExamples:\n"
            + HelpExampleCli("getnewbillpubkey", "")
            + HelpExampleRpc("getnewbillpubkey", "")
        );
    ...
    if (!pwallet->GetKeyFromPool(pubkey))
        throw JSONRPCError(RPC_MISC_ERROR, "Keypool ran out, please call keypoolrefill first!");

    UniValue response(UniValue::VOBJ);
    response.pushKV("pubkey", HexStr(pubkey.begin(), pubkey.end()));
    return response;
}

UniValue listmybills(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    for (const CBill& bill : pbilltree->GetBills()) {
        bool fHolder = pwallet->HaveKey(CPubKey(bill.vchHolderPubKey).GetID());
        bool fDrawer = pwallet->HaveKey(CPubKey(bill.vchDrawerPubKey).GetID());
        bool fAcceptor = pwallet->HaveKey(CPubKey(bill.vchAcceptorPubKey).GetID());
        if (!fHolder && !fDrawer && !fAcceptor)
            continue;
        ...
        if (fHolder) roles.push_back("holder");
        if (fDrawer) roles.push_back("drawer");
        if (fAcceptor) roles.push_back("acceptor");
        obj.pushKV("roles", roles);
